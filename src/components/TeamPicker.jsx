  ...
  return <section className={`team-picker ${selectedTeam ? 'team-picker--chosen' : ''}`} aria-label="Selecciona un equipo">
    ...
    <form onSubmit={onSearch} className="search-form">
      ...
      <div className="search-row">
        ...
        <button className="button button--primary" type="submit" disabled={isLoading}>{isLoading ? 'Buscando...' : 'Crear XI'}</button>
      </div>
    </form>
    ...
    {teams.length > 0 && <div className="team-results">{teams.map((item) => <button className="team-result" key={item.team.id} onClick={() => onSelectTeam(item)}><img src={item.team.logo} alt="" /><span>{item.team.name}</span><small>{item.team.isWomen && <b className="team-result__gender">Femenino</b>}{item.venue?.name || item.country}</small></button>)}</div>}
  </section>
}
