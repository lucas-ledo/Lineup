...
export function LineupPitch({
  ...
  return <div className="pitch-panel" ref={captureRef}>
    ...
    <div className="pitch" aria-label="Campo de fútbol, selecciona una posición vacía">
      ...
      {slots.map((slot) => {
        ...
        return <div key={slot.id} data-slot-id={slot.id} className={`slot ${player ? 'slot--filled' : ''} ${selectedSlot === slot.id ? 'slot--selected' : ''}`} style={{ left: `${slot.x}%`, top: `${slot.y}%` }} onDragOver={onAllowDrop} onDrop={(event) => onDropSlot(event, slot.id)}>
          {player ? <button className={`field-card ${draggedPlayer?.id === player.id ? 'field-card--dragging' : ''} ${touchDrag?.player.id === player.id ? 'field-card--touch-moving' : ''}`} data-shirt={player.number ?? '—'} draggable onDragStart={(event) => onDragStart(event, player)} onDragEnd={onDragEnd} onPointerDown={(event) => onPointerDown(event, player)} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerCancel} onClick={() => { if (!shouldSuppressTap()) onClearPlayer(player.id) }} title={`Arrastra a ${player.name} a otra posición. Pulsa para quitarlo.`}><Avatar player={player} /><span className="field-number">#{player.number ?? '—'}</span><span className="field-details"><strong>{player.name}</strong><small>{slot.label}</small></span><img className="field-crest" src={player.club?.logo || team?.logo} alt="" /></button> : <button className="empty-slot" onClick={() => onSelectSlot(slot.id)} title={`Elegir ${slot.label}`}><span className="slot-plus">+</span><span className="slot-label">{slot.label}</span></button>}
        </div>
      })}
    </div>
    <p className="hint" data-html2canvas-ignore="true">{selectedSlot ? `Posición seleccionada: ${selectedSlotLabel}. Elige o arrastra un jugador de la lista.` : 'Pulsa una posición vacía o arrastra un jugador al campo. Pulsa un titular para quitarlo.'}</p>
  </div>
}
