...
export function TransferMarket({
  ...
  return <section className="transfer-panel">
    ...
    {transferLog.length > 0 && <div className="transfer-log">{transferLog.map((transfer) => <article className={`transfer-log-item transfer-log-item--${transfer.type}`} key={transfer.id}><Avatar player={transfer.player} small /><div><strong>{transfer.player.name}</strong><span>{transfer.type === 'signing' ? 'Fichaje' : 'Venta'} · {euro.format(transfer.price)}</span></div><button onClick={() => onUndoTransfer(transfer)}>Deshacer</button></article>)}</div>}
    {isOpen && <div className="market-body">
      ...
      {selectedTeam && <div className="market-club"><img src={selectedTeam.logo} alt="" /><span>Plantilla de {selectedTeam.name}</span><button onClick={onClearSelectedTeam}>×</button></div>}
      ...
      {players.length > 0 && <div className="market-players">{players.map((player) => {
        ...
        return <article className="market-player" key={player.id}><Avatar player={player} /><div><strong>{player.name}</strong><span>{positionNames[player.position] || player.position} · #{player.number ?? '—'}</span>{player.club?.name && <small className="market-player-club">{player.club.name}</small>}</div>{isAlreadyInRoster ? <small>En plantilla</small> : isEditing ? <div className="price-editor"><label>€<input autoFocus type="number" min="1" step="1" value={signingDraft.price} onChange={(event) => onSigningDraftChange((current) => ({ ...current, price: event.target.value }))} aria-label={`Precio de fichaje de ${player.name} en euros`} /></label><button onClick={onConfirmSigning}>Fichar</button><button className="cancel-price" onClick={() => onSigningDraftChange(null)}>×</button></div> : <button className="buy-player" onClick={() => onStartSigning(player)}>Fichar</button>}</article>
      })}</div>}
    </div>}
  </section>
}
